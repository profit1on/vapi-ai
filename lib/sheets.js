...
export const processEndCallReport = async (report) => {
    ...
    // Update the lead status separately (not in batch)
    await updateLeadInfo(rowIndex + 1, 'called', phoneCallProviderId); // Update to "called" status
...
export const updateLeadInfo = async (rowIndex, status, phoneCallProviderId) => {
    const auth = await getClient();
    const sheets = google.sheets({ version: 'v4', auth });
