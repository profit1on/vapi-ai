...
export const updateCallDetails = async (rowIndex, duration, price) => {
    ...
    // Update the call price in column H
    const priceRange = `'Lead list'!H${rowIndex}`; // Assuming call price is stored in column H
    await sheets.spreadsheets.values.update({
        spreadsheetId,
        range: priceRange,
        valueInputOption: 'RAW',
...
export const processEndCallReport = async (report) => {
    ...
    // Update the lead status, phoneCallProviderId, callId, and ended reason
