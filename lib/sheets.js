    ...
    const updates = [
        {
            ...
            values: ['called'], // Update to "called" status
        },
        {
...
export const updateLeadInfo = async (rowIndex, status, phoneCallProviderId, callId) => {
    ...
    const statusRange = `Lead list!F${rowIndex + 1}`; // Assuming status is in column F
    ...
    const providerIdRange = `Lead list!G${rowIndex + 1}`; // Assuming phoneCallProviderId is stored in column G
    await sheets.spreadsheets.values.update({
        spreadsheetId,
    ...
    const callIdRange = `Lead list!H${rowIndex + 1}`; // Assuming callId is stored in column H
    await sheets.spreadsheets.values.update({
        spreadsheetId,
        range: callIdRange,
        valueInputOption: 'RAW',
...
export const updateCellM = async (rowIndex, value) => {
    ...
    const cellMRange = `Lead list!M${rowIndex + 1}`; // Assuming you want to update column M
    await sheets.spreadsheets.values.update({
        spreadsheetId,
        range: cellMRange,
        valueInputOption: 'RAW',
