// pages/api/makeAppointment.js
import { updateCellQ, getLeads } from '../../lib/sheets'; // Import the function to update Google Sheets
...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        console.log('Testing makeAppointment Received request body:', req.body); // Log the incoming request body
        ...
        // Extract makeAppointment from the tool call's function arguments
        const makeAppointmentFromToolCall = firstToolCall.function?.arguments?.makeAppointment;

        // Use makeAppointmentFromToolCall or fallback to customer.name, or set to empty string if both are undefined
        const argumentToUpdate = makeAppointmentFromToolCall || message.customer?.name || ''; // Set to empty string if not found

        // Log the argument to update for verification
        console.log(`Updating cell Q${rowIndex + 1} with argument: ${argumentToUpdate}`);
        ...
        try {
            ...
            if (updatesBatch.length >= BATCH_SIZE) {
                await Promise.all(updatesBatch.map(async (update) => {
                    await updateCellQ(update.rowIndex, update.value);
                }));
