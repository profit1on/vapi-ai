// pages/api/lostAnything.js
import { updateCellO, getLeads } from '../../lib/sheets'; // Import the function to update Google Sheets
...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        console.log('Testing lostAnything Received request body:', req.body); // Log the incoming request body
        ...
        // Extract lostAnything from the tool call's function arguments
        const lostAnythingFromToolCall = firstToolCall.function?.arguments?.lostAnything;

        // Use lostAnythingFromToolCall or fallback to customer.name, or set to empty string if both are undefined
        const argumentToUpdate = lostAnythingFromToolCall || message.customer?.name || ''; // Set to empty string if not found

        // Log the argument to update for verification
        console.log(`Updating cell O${rowIndex + 1} with argument: ${argumentToUpdate}`);
        ...
        try {
            ...
            if (updatesBatch.length >= BATCH_SIZE) {
                await Promise.all(updatesBatch.map(async (update) => {
                    await updateCellO(update.rowIndex, update.value);
                }));
