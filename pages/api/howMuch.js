// pages/api/howMuch.js
import { updateCellP, getLeads } from '../../lib/sheets'; // Import the function to update Google Sheets
...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        console.log('Testing howMuch Received request body:', req.body); // Log the incoming request body
        ...
        // Extract howMuch from the tool call's function arguments
        const howMuchFromToolCall = firstToolCall.function?.arguments?.howMuch;

        // Use howMuchFromToolCall or fallback to customer.name, or set to empty string if both are undefined
        const argumentToUpdate = howMuchFromToolCall || message.customer?.name || ''; // Set to empty string if not found

        // Log the argument to update for verification
        console.log(`Updating cell P${rowIndex + 1} with argument: ${argumentToUpdate}`);
        ...
        try {
            ...
            if (updatesBatch.length >= BATCH_SIZE) {
                await Promise.all(updatesBatch.map(async (update) => {
                    await updateCellP(update.rowIndex, update.value);
                }));
